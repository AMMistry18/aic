"""Simulator-ground-truth collector for a separate SFP plug-pose model.

Run the simulator with ``ground_truth:=true`` so ``cable_N/sfp_tip_link`` is
available, then select this policy.  Each collection run is assigned wholly to
train, validation, or test so adjacent frames from the same randomized scene
...
OUTPUT_DIR = Path(
    os.path.expanduser(
        os.environ.get(
            "AIC_SFP_PLUG_POSE_OUTPUT_DIR",
            "~/aic_perception_data/sfp_plug_pose",
        )
    )
).resolve()
CAMERA_NAMES = ("left_camera", "center_camera", "right_camera")
VIEWPOINTS_PER_TRIAL = int(os.environ.get("AIC_SFP_PLUG_POSE_VIEWPOINTS", "1"))
FRAMES_PER_VIEWPOINT = int(os.environ.get("AIC_SFP_PLUG_POSE_FRAMES_PER_VIEW", "3"))
SETTLE_SECONDS = float(os.environ.get("AIC_SFP_PLUG_POSE_SETTLE_S", "0.6"))
FRAME_INTERVAL_SECONDS = float(os.environ.get("AIC_SFP_PLUG_POSE_FRAME_INTERVAL_S", "0.18"))
INITIAL_SETTLE_SECONDS = float(
    os.environ.get("AIC_SFP_PLUG_POSE_INITIAL_SETTLE_S", "2.0")
)
RETURN_SETTLE_SECONDS = float(
    os.environ.get("AIC_SFP_PLUG_POSE_RETURN_SETTLE_S", "1.0")
)
MIN_VISIBLE_KEYPOINTS = 6
BBOX_PADDING = 0.28


def _split_for_trial(run_number: int, trial_number: int) -> str:
    """80/10/10 whole-trial train/val/test assignment."""

    cycle = ((int(run_number) - 1) * 100_000 + int(trial_number) - 1) % 10
    if cycle == 9:
        return "test"
    if cycle == 8:
        return "val"
...
def sample_viewpoints(count: int, rng: random.Random) -> list[dict[str, float]]:
    """Sample safe camera offsets and wrist rotations around the initial grasp."""

    canonical = [
        {"dx": 0.0, "dy": 0.0, "dz": 0.0, "rx": 0.0, "ry": 0.0, "rz": 0.0},
        {"dx": 0.025, "dy": 0.0, "dz": 0.025, "rx": 0.10, "ry": 0.0, "rz": 0.0},
        {"dx": -0.025, "dy": 0.0, "dz": 0.025, "rx": -0.10, "ry": 0.0, "rz": 0.0},
        {"dx": 0.0, "dy": 0.025, "dz": 0.035, "rx": 0.0, "ry": 0.10, "rz": 0.0},
    ]
        ...
        viewpoints.append(
            {
                ...
                "dy": rng.uniform(-0.04, 0.04),
                "dz": rng.uniform(-0.005, 0.075),
                "rx": rng.uniform(-0.22, 0.22),
                "ry": rng.uniform(-0.22, 0.22),
                "rz": rng.uniform(-0.16, 0.16),
            }
        )
...
class DataCollectorSfpPlugPoseGT(Policy):
    """Collect RGB/YOLO labels from the exact SFP tip ground-truth TF."""
    ...
    def __init__(self, parent_node):
        ...
        self._rng = random.Random(1009 * self._run_number)
        self._prepare_directories()
        write_dataset_yaml(OUTPUT_DIR)
        self.get_logger().info(
            "SFP plug GT collector initialized | "
            f"run={self._run_number} split=per-trial-80/10/10 "
            f"views={VIEWPOINTS_PER_TRIAL} frames/view={FRAMES_PER_VIEWPOINT} "
            f"output={OUTPUT_DIR}"
        )
    ...
    def _prepare_directories(self):
        ...
        if os.environ.get("AIC_SFP_PLUG_POSE_SAVE_DEBUG", "0") == "1":
    ...
    def _tip_frame_candidates(task: Task) -> list[str]:
        ...
        candidates = [
            requested,
            f"{task.cable_name}/sfp_tip_link",
    ...
    def _capture_observation(
        ...
        frame_id = (
            f"r{self._run_number:04d}_t{self._trial_number:04d}_"
            f"v{viewpoint_index:03d}_s{sample_index:02d}"
        )
        metadata: dict[str, Any] = {
            ...
            "viewpoint": viewpoint,
            "local_keypoints_m": SFP_PLUG_LOCAL_KEYPOINTS_M.tolist(),
            "cameras": {},
        }
        saved = 0
        ...
        for camera_name in CAMERA_NAMES:
            ...
            pixels, in_front = project_keypoints(
                SFP_PLUG_LOCAL_KEYPOINTS_M, camera_from_tip, K
            )
            ...
            image_path = OUTPUT_DIR / "images" / self._split / image_name
            label_path = OUTPUT_DIR / "labels" / self._split / label_name
            ...
            metadata["cameras"][camera_name] = {
                ...
                "frame_id": f"{camera_name}:{image_msg.header.stamp.sec}."
                f"{int(image_msg.header.stamp.nanosec):09d}",
    ...
    def insert_cable(
        ...
        if str(task.port_type).lower() != "sfp":
            self.get_logger().info(
                f"Skipping non-SFP task while collecting plug pose: {task.port_type}"
            )
            return True
        self._trial_number += 1
        self._split = _split_for_trial(self._run_number, self._trial_number)
        self.sleep_for(INITIAL_SETTLE_SECONDS)
        initial_observation = get_observation()
        if initial_observation is None:
            self.get_logger().error("No initial observation; SFP plug collection skipped")
            return False
        ...
        for viewpoint_index, viewpoint in enumerate(viewpoints):
            ...
            rotation_size = math.sqrt(
                viewpoint["rx"] ** 2 + viewpoint["ry"] ** 2 + viewpoint["rz"] ** 2
            )
            translation_size = math.sqrt(
                viewpoint["dx"] ** 2 + viewpoint["dy"] ** 2 + viewpoint["dz"] ** 2
            )
            self.sleep_for(max(SETTLE_SECONDS, 7.0 * translation_size + 1.2 * rotation_size))
            ...
            self.get_logger().info(
                f"SFP plug GT {viewpoint_index + 1}/{len(viewpoints)} | "
                f"saved camera images={saved}"
            )
        ...
        message = (
            f"SFP plug GT collection complete: saved={saved} split={self._split} "
            f"dataset={yaml_path}"
        )
