...
metrics ONLY under --out (default RL/output/student_teacher/residual_funnel_lvl1).
...
Example (FULL run -- launch this yourself; the smoke uses tiny values):
  WANDB_MODE=offline .pixi/envs/default/bin/python -m RL.student_teacher.train_residual_teacher \
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="RL/output/student_teacher/residual_funnel_lvl1")
...
def _certify(model_path, episodes, seed, residual_scale, level, obs_noise=0.0):
    ...
    from RL.student_teacher.residual_env import make_residual_teacher_env
