...
def main():
  ...
  try:
    ...
    robot_keywords = [
        "tabletop", "shoulder", "upper_arm", "upperarm", "forearm",
        "wrist", "ati", "gripper", "hande", "cam_mount", "axia",
        "basler", "base-", "base1", "_base.stl", "base_eSeries",
    ]
    env_keywords = [
        "task_board", "sc_port", "nic_card", "plug", "sfp_module",
        "enclosure", "floor", "walls", "light",
    ]
    ...
    id_l1 = mujoco.mj_name2id(orig_model, mujoco.mjtObj.mjOBJ_BODY, "link_1")  # pytype: disable=wrong-arg-types
    ...
    def copy_body_recursive(
      ...
      for defer_name, (target_parent, defer_pos, defer_quat) in reparent_map.items():
    ...
    def traverse_find_links(body, target_plugin):
      ...
      is_cable_body = (
          body.name.startswith("cable_end_") or
          body.name.startswith("cable_connection_") or
          body.name == "sc_plug_link"
      )
