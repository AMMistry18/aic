#
# CableInsertionPolicy -- perception-guided SFP and SC insertion policy.
#
...
#
# Module path for aic_model:  -p policy:=aic_model.CableInsertionPolicy
#
# Key imports outside this file are:
#   * PerceptionCore -- the raw YOLO wrapper (best.pt / best_sc_mouth_pose.pt)
#   * rl_insert_contract -- shared pose and action geometry retained by the
#     deterministic insertion controller.
#
import itertools
import os
import time
...
# ----------------------------- configuration -------------------------------
MODEL_PATH = os.environ.get("RL_INSERT_MODEL", "")
...
CONTROL_MODE = "script"
# Retries need room to run: one attempt is align (<=15 s) plus seating
# (~10-25 s), and a retract-and-retry cycle adds ~12 s, so 45 s bought exactly
# zero retries.  The scorer's 60 s is only where the duration bonus reaches zero
# -- not a cutoff -- so trading ~3 remaining duration points for a converted
# full insertion (75 points) is strongly net positive.
#
# This is the *controller* budget for both SFP and SC: both retry ladders are
...
SCRIPT_BIAS_X_M = float(os.environ.get("RL_INSERT_SCRIPT_BIAS_X_M", "-0.0008"))     # -0.8 mm (-X, was -1.0)
SCRIPT_BIAS_Y_M = float(os.environ.get("RL_INSERT_SCRIPT_BIAS_Y_M", "0.007"))       # +7.0 mm (+Y)
...
# Retained as legacy configuration metadata, but the script-only controller no
# longer calls lateral nudge recovery after centering.
SCRIPT_NUDGE_ENABLE = os.environ.get(
    "RL_INSERT_SCRIPT_NUDGE_ENABLE", "0").strip().lower() in ("1", "true", "yes")
...
class CableInsertionPolicy(VisualGapRecoveryMixin, Policy):
    ...
    def __init__(self, parent_node):
        ...
        # V50 requires a fresh, directly observed plug pose. Missing plug
        # weights fail lifecycle configuration instead of falling back to a
        # fixed grasp transform.
        configure_v50(self)

        # Script-only mode does not import torch, require an actor artifact, or
        # execute learned control.
        self._torch = None
        self._model = None
        log.info("[script] SCRIPT-ONLY: RL actor loading and inference are disabled")
    ...
    def _run(self, task, get_observation, move_robot, send_feedback):
        ...
        # V50 direct plug-to-port control. The legacy fixed-bias script remains
        # in the source for provenance but is unreachable in this release.
    ...
    def _run_script(self, get_observation, move_robot, send_feedback, *,
        ...
        for dstep in range(MAX_RL_STEPS):
            ...
            # Detect a stall for diagnostics. In script-only mode this never hands
            # off to an actor or starts a lateral search: the straight compliant
            # seating command below remains in control.
            ...
            if depth_stalled or force_jam:
                ...
                if dstep % 10 == 0:
                    log.info(
                        f"[script] SCRIPT-ONLY {trigger} at depth={depth*1000:.1f}mm "
                        f"lateral={lateral*1000:.2f}mm force={f_mag:.2f}N -- "
                        "continuing straight force-limited seating; no RL or search")
                deepest = depth
                stall_steps = 0
                stall_depth = float("nan")
                stall_lat_min, stall_lat_max = np.inf, -np.inf
