# Insertion-policy Docker ground truth

This is the reproducible Phase 1 image used to build and install the current
SFP and SC insertion policy.
...
- copies the canonical policy package, perception code, and model weights; and
- starts `aic_model.insertion.InsertionPolicy`.

Dependency installation precedes the policy copy, so policy-only edits reuse
...
./docker/aic_model/build_insertion_policy.sh
```
...
from aic_model.insertion.sfp_controller import run_sfp_insertion
...
GripperMaskBank()
print(\"SFP + SC insertion imports and weights OK\")
"'
```

The entrypoint bytes must end in `0a`, not `0d 0a`. The configured policy must
be `aic_model.insertion.InsertionPolicy`.
...
The first recorded `InsertCable` execution reached the policy but aborted after about
seven seconds. That physical insertion result is not yet a validated success;
the `aic_model` service logs must be used to identify the exact abort
