...
This is the SC counterpart to :mod:`sfp_controller`.  It reuses that module's
...
The tip is located from vision, exactly as SFP does for SFP: every run starts
by measuring ``sc_tip_link`` with :class:`~aic_model.insertion.sc_plug_pose.ScPlugPoseEstimator`
...
``aic_model/aic_model/insertion/sc_plug_pose_geometry.py``
...
# Deliberately local rather than imported from sfp_controller: these are that
...
def _normalize_event(value: object) -> str:
    ...
    while the task names only ``nic_card_mount_0/sfp_port_0``.  SFP's normaliser
...
#
# Do NOT reuse InsertionPolicy's CALIB_PLUG_FRAMES: it is SFP-only
...
class SCConfig:
    ...
    # Proportional wrench alignment, same law as the fixed SFP one: a low-pass of
...
def sc_grasp_transform(policy):
    ...
    ``prime_sfp_plug_pose`` populates ``_sfp_grasp_transform`` for SFP.
    """
...
def sc_tip_from_tcp(policy, tcp_pos, tcp_quat):
    ...
    Mirrors ``sfp_tip_from_tcp``: before priming (unit tests, or the calib-dump
...
def sc_tcp_pose_for_tip(policy, tip_pos, tip_rotation):
    ...
    ``tcp_pose_for_sc_tip`` does.  Deliberately not SFP's
...
def next_sc_depth(
    ...
    Deliberately a separate function from ``sfp_controller.next_persistent_depth``
...
def _sc_calib_dump_enabled() -> bool:
    ...
    InsertionPolicy snapshots CALIB_DUMP into a module constant on import, which makes
...
def dump_sc_grasp_calibration(policy, task) -> bool:
    ...
    InsertionPolicy's ``_dump_grasp_calibration`` is unreachable on the SC path -- the
...
class ScInsertionController:
    ...
    # (sfp_controller.max_wedge_retries).  The ladder alternates cheap and full
    ...
    def _tcp_target(self, tip_pos, tip_rotation):
        ...
        from .contract import matrix_to_quat
    ...
    def _alignment_sample(self, observation, depth, force, acc_lat, acc_tilt):
        ...
        Identical in form to the fixed SFP law.  The SFP field logs showed an
...
def configure_sc_plug_pose(policy) -> bool:
    ...
    lifecycle configure like ``configure_sfp_controller``: the mainline ``InsertionPolicy`` has no
...
def prime_sc_plug_pose(policy, get_observation, move_robot) -> bool:
    ...
    if not configure_sc_plug_pose(policy):
        return False
    from .sfp_plug_pose import stamp_to_seconds
    from .sfp_controller import _observation_stamp_s, _plug_views_from_observation
...
def run_sc_insertion(policy, task, get_observation, move_robot, send_feedback) -> bool:
    ...
    # Measure the grasped plug BEFORE port perception, mirroring SFP: the port
