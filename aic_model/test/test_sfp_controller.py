...
from aic_model.insertion.sfp_controller import (  # noqa: E402
    HARD_FAILURE,
    INSERT_DEPTH_M,
...
import aic_model.insertion.sfp_controller as sfp_controller_module  # noqa: E402
from aic_model.insertion.contract import port_frame  # noqa: E402


def test_sfp_config_bounds_force_and_seating():
    config = SfpConfig().validated()
    ...
    with pytest.raises(ValueError, match="hard abort"):
        SfpConfig(force_abort_n=19.0).validated()
    with pytest.raises(ValueError, match="overtravel"):
        SfpConfig(seat_overtravel_m=0.009).validated()
    with pytest.raises(ValueError, match="release decay"):
        SfpConfig(seat_align_release_decay=1.5).validated()
    ...
    with pytest.raises(ValueError, match="port clearance"):
        SfpConfig(seat_align_max_lat_m=0.0015).validated()
    with pytest.raises(ValueError, match="slew"):
        SfpConfig(seat_align_max_step_m=0.0).validated()
    ...
    with pytest.raises(ValueError, match="envelope"):
        SfpConfig(wedge_recovery_max_lat_m=0.0011).validated()
...
def test_controller_wrench_uses_the_documented_sensor_frame_and_tip_lever_arm():
    ...
    force = np.array([5.0, -2.0, 0.0])
    controller = object.__new__(SfpInsertionController)
...
def test_persistent_depth_accumulates_force_lead_while_stalled():
    ...
    # v49 repeatedly requested only current+~1mm.  SFP retains an absolute
...
def test_persistent_depth_allows_bounded_overtravel_near_insert_depth():
    config = SfpConfig(seat_overtravel_m=0.003).validated()
...
def test_retract_pull_lead_must_stay_under_the_hard_abort():
    with pytest.raises(ValueError, match="hard force abort"):
        SfpConfig(retract_pull_lead_m=0.040).validated()  # 500 N/m * 40mm = 20 N
    with pytest.raises(ValueError, match="positive"):
        SfpConfig(retract_pull_lead_m=0.0).validated()


def test_wall_progress_watch_uses_elapsed_time_not_loop_count():
    watch = WallProgressWatch.start(0.010, now=100.0, config=SfpConfig())
    for _ in range(500):
...
def test_priming_uses_direct_fresh_plug_pose_before_port_selection():
    ...
    policy = SimpleNamespace(
        _sfp_grasp_transform=(np.ones(3), np.eye(3)),
        _sfp_pending_world_plug=None,
        _sfp_plug_estimator=Estimator(),
        _sfp_config=SfpConfig(),
        _parent_node=parent,
    ...
    assert prime_sfp_plug_pose(policy, lambda: observation, lambda *_args: None)
    assert policy._sfp_pending_world_plug is world_estimate
    np.testing.assert_allclose(policy._sfp_grasp_transform[0], world_estimate.position_world)
...
def test_priming_failure_clears_stale_grasp_and_fails_closed():
    ...
    policy = SimpleNamespace(
        _sfp_grasp_transform=(np.ones(3), np.eye(3)),
        _sfp_pending_world_plug=object(),
        _sfp_plug_estimator=Estimator(),
        _sfp_config=SfpConfig(),
    ...
    assert not prime_sfp_plug_pose(policy, lambda: observation, lambda *_args: None)
    assert policy._sfp_grasp_transform is None
    assert policy._sfp_pending_world_plug is None
...
class _SequenceHarness(SfpInsertionController):
    def __init__(
        self,
        outcomes,
        *,
        ...
        self.config = (config or SfpConfig()).validated()
...
def test_failed_plug_refresh_clears_stale_grasp_transform():
    ...
    controller.log = _Log()
    controller.last_observation_stamp = 12.0
    controller.policy = SimpleNamespace(_sfp_grasp_transform=("stale", "pose"))
    controller._wait_new_observation = lambda **_kwargs: None

    assert controller._refresh_plug_pose_after_retract() is False
    assert controller.policy._sfp_grasp_transform is None


class _AlignHarness(SfpInsertionController):
...
def test_verified_recovery_and_live_wrench_correction_share_one_clearance_cap():
    harness = object.__new__(SfpInsertionController)
    harness.config = SfpConfig().validated()
...
def test_force_guided_deep_recovery_accepts_only_a_measured_improvement(monkeypatch):
    ...
    monkeypatch.setattr(sfp_controller_module.time, "monotonic", lambda: clock[0])
...
def test_missing_plug_pose_fails_closed_before_any_motion():
    ...
    assert harness.trace == [
        ("feedback", "SFP fresh plug-to-port perception"),
        "fresh-plug",
        "safe-hold",
    ]
...
def test_wedge_retry_can_be_disabled_and_capped():
    disabled = _SequenceHarness(
        [WEDGED, SEATED], config=SfpConfig(wedge_retry_enable=False)
    )
    assert disabled.run() is False
    assert "retract" not in disabled.trace

    capped = _SequenceHarness(
        [WEDGED] * 5, config=SfpConfig(max_wedge_retries=2)
    )
    assert capped.run() is False
    assert capped.trace.count("retract") == 2


def test_dockerfile_copies_the_canonical_policy_package():
    dockerfile = (REPO_ROOT / "docker" / "aic_model" / "Dockerfile").read_text(
        encoding="utf-8"
    )
    assert "COPY aic_model/aic_model /tmp/aic_model" in dockerfile
    assert "cp -a /tmp/aic_model/." in dockerfile
    assert "v50_overlay" not in dockerfile


def test_canonical_runtime_contains_the_latest_deployed_integration():
    policy = (
        REPO_ROOT
        / "aic_model"
        / "aic_model"
    ...
    host = (REPO_ROOT / "aic_model" / "aic_model" / "aic_model.py").read_text(
        encoding="utf-8"
    )

    assert "class InsertionPolicy(VisualGapRecoveryMixin, Policy)" in policy
    assert "configure_sfp_controller(self)" in policy
    assert "return run_sfp_insertion(" in policy
    assert "ACTION_TIME_BUDGET_S" in policy
    assert '"/scoring/insertion_event"' in host
    assert "RL_INSERT_SC_MISS_ABORTS" in host
...
def test_release_dockerfile_disables_bias_and_pins_safety():
    ...
    # so a seat-force retune in SfpConfig is a no-op in the image unless these
