...
and the output paths differ from the SFP trainer; the augmentation and loss
weights are deliberately identical so the two runs stay comparable.
...
def _parse_args():
    ...
    parser.add_argument(
        "--data",
        type=Path,
        default=dataset_root / "aic_sc_plug_pose.yaml",
    )
    ...
    parser.add_argument("--name", default="sc_plug_pose_yolo11s_960")
    parser.add_argument(
        "--final-weights",
        type=Path,
        default=(
            ...
            / "ros"
            / "weights"
            / "best_sc_plug_pose.pt"
        ),
    )
    parser.add_argument(
        "--metrics-out",
        type=Path,
        default=dataset_root / "reports" / "train_sc_plug_pose.json",
    )
...
# One class id, one bbox (4), then eight keypoints of (x, y, visibility).
EXPECTED_LABEL_TOKENS = 5 + 3 * 8


def _check_label_shape(data_yaml: Path, limit: int = 200) -> None:
    """Fail before opening a GPU if the labels are not eight-keypoint poses.

    A silently mis-shaped label set trains a model whose keypoint order does
    not match ``SC_PLUG_LOCAL_KEYPOINTS_M``, which produces a confident but
    wrong rigid fit at runtime.  That is exactly the failure the fail-closed
    estimator cannot detect, so it is checked here instead.
    """

    root = data_yaml.parent
    for split in ("train", "val"):
        labels = sorted((root / "labels" / split).glob("*.txt"))[:limit]
        for label in labels:
            tokens = label.read_text(encoding="utf-8").split()
            if len(tokens) != EXPECTED_LABEL_TOKENS:
                raise RuntimeError(
                    f"{label} has {len(tokens)} tokens; expected "
                    f"{EXPECTED_LABEL_TOKENS} for an 8-keypoint SC plug pose"
                )
...
def main():
    ...
    results = model.train(
        ...
        shear=2.0,
        perspective=0.0002,
        # The eight SC keypoints are labelled in a fixed body order, and the
        # duplex housing is close to mirror-symmetric in appearance.  A
        # horizontal flip would swap the near/rear plane assignment in a way
        # flip_idx cannot express for a 3D cuboid, so both flips stay off.
        fliplr=0.0,
        flipud=0.0,
        mosaic=0.5,
        mixup=0.05,
